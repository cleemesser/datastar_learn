...
There are many backend servers one can use with datastar, that is one of its
advantages. I am going to illustrate these patterns assuming a fastapi-like
interface. This was actually developed with django-bolt, but that is still
pre-production level so I will use fastapi as the standard.
...
# should this also be async def?
def sse_response(generator_function):
    return DatastarResponse(generator_function())


# example usage, note some headers may also need to be set
...
async def updates(signals: ReadSignals):
    ...
    print(signals)
    return sse_response(time_generator)
